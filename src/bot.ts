...
let lastNotifiedGameStartName: string;
let lastNotifiedGameStartTime = 0;
let lastNotifiedGameStopName: string;
...
bot.on("presenceUpdate", (other, oldPresence) => {
  if (other instanceof Eris.Relationship) return;

  // Botが投稿するためのTextChannelを取得
  // TextChannelが１つの場合を想定しています。
  // 複数ある場合はchannel.id等で判別できます。
  const textChannel = other.guild.channels.find(
    (channel: any) => channel.id === notifyChannelId
  );
  const userName = other.user.username;

  if (!textChannel || !userName || !other.game) return;

  // ゲームが始まった時
  if (/MTG/.test(other.game.name) || /PLAYERUNKNOWN/.test(other.game.name)) {
    if (
      other.game.name !== lastNotifiedGameStartName ||
...
bot.on("voiceChannelJoin", (member, newChannel) => {
  const textChannel = newChannel.guild.channels.find(
    (channel) => channel.id === notifyChannelId
  );
